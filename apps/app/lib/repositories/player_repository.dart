...
class PlayerRepository {
  ...
  Future<PlayerTeamModel?> getByTeamAndPlayer({required int teamId, required int playerId}) async {
    try {
      final res = await _dio.get('/player-teams/', queryParameters: {'team_id': teamId, 'player_id': playerId});
      final list = (res.data as List);
      if (list.isEmpty) return null;
      return PlayerTeamModel.fromJson(list.first);
    } on DioException catch (e) { throw ApiException.fromDioError(e); }
  }
  ...
  Future<PlayerModel> update(int playerId, {String? name, String? dni}) async {
    try {
      final data = <String, dynamic>{};
      if (name != null) data['name'] = name;
      if (dni != null) data['dni'] = dni;
      final res = await _dio.patch('/players/$playerId', data: data);
      return PlayerModel.fromJson(res.data);
    } on DioException catch (e) { throw ApiException.fromDioError(e); }
  }
  ...
  Future<PlayerTeamModel> updatePlayerTeam(int ptId, {int? number}) async {
    try {
      final data = <String, dynamic>{};
      if (number != null) data['number'] = number;
      final res = await _dio.patch('/player-teams/$ptId', data: data);
      return PlayerTeamModel.fromJson(res.data);
    } on DioException catch (e) { throw ApiException.fromDioError(e); }
  }
}
