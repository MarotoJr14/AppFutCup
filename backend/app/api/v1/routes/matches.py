...
def create(data: MatchCreate, db: Session = Depends(get_db), _: User = Depends(require_org)):
    return MatchService(db).create(data)


@router.patch("/{match_id}", response_model=MatchOut)
def update(match_id: int, data: MatchUpdate, db: Session = Depends(get_db), _: User = Depends(require_org)):
    return MatchService(db).update(match_id, data)


@router.delete("/{match_id}", status_code=204)
def delete(match_id: int, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    MatchService(db).delete(match_id)
