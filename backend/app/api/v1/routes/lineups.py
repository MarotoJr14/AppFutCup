...
def create(data: LineupCreate, db: Session = Depends(get_db), _: User = Depends(require_org)):
    return LineupService(db).create(data)


@router.post("/bulk", response_model=list[LineupOut], status_code=201)
def bulk_create(data: LineupBulkCreate, db: Session = Depends(get_db), _: User = Depends(require_org)):
    return LineupService(db).bulk_create(data)


@router.patch("/{lineup_id}", response_model=LineupOut)
def update(lineup_id: int, data: LineupUpdate, db: Session = Depends(get_db), _: User = Depends(require_org)):
    return LineupService(db).update(lineup_id, data)


@router.delete("/{lineup_id}", status_code=204)
def delete(lineup_id: int, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    LineupService(db).delete(lineup_id)
