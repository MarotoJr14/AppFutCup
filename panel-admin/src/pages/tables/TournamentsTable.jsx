...
export default function TournamentsTable() {
  ...
  const handleImport = async (file, format) => {
    return await importData(file, format, async (row) => {
      await api.post('/tournaments', { name: row.name, place: row.place, date_ini: row.date_ini, date_end: row.date_end, is_active: row.is_active === 'true' || row.is_active === 'Sí' || row.is_active === true })
    })
  }
  ...
  return (
    <div>
      ...
      />
      {modal === 'view' && selected && (<Modal title={selected.name} onClose={() => setModal(null)}><div className="space-y-2">{COLUMNS.map(c => (<div key={c.key} className="flex gap-2"><span className="text-hint text-sm w-28 shrink-0">{c.label}:</span><span className="text-white text-sm">{c.render ? c.render(selected[c.key]) : String(selected[c.key] ?? '—')}</span></div>))}</div></Modal>)}
      {modal === 'create' && (<Modal title="Crear torneo" onClose={() => setModal(null)}><GenericForm fields={FIELDS} onSubmit={handleCreate} onCancel={() => setModal(null)} submitLabel="Crear torneo" /></Modal>)}
      {modal === 'edit' && selected && (<Modal title={`Editar: ${selected.name}`} onClose={() => setModal(null)}><GenericForm fields={FIELDS} initial={{ ...selected, date_ini: selected.date_ini?.slice(0,16) || '', date_end: selected.date_end?.slice(0,16) || '', is_active: String(selected.is_active) }} onSubmit={handleEdit} onCancel={() => setModal(null)} submitLabel="Guardar cambios" /></Modal>)}
      {modal === 'import' && (<ImportModal title="Tournaments" templateHeaders={TEMPLATE_HEADERS} onImport={handleImport} onClose={() => setModal(null)} />)}
