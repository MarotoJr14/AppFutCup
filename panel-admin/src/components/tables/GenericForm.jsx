        ...
        <div key={f.key}>
          ...
          {f.type === 'select' ? (
            <select value={form[f.key] ?? ''} onChange={e => set(f.key, e.target.value)}
              className="input-base bg-surface-alt">
            ...
            <textarea value={form[f.key] ?? ''} onChange={e => set(f.key, e.target.value)}
              rows={3} className="input-base resize-none" placeholder={f.placeholder} />
          ) : f.type === 'datetime' ? (
            <input type="datetime-local" value={form[f.key] ?? ''}
              onChange={e => set(f.key, e.target.value)} className="input-base" />
          ) : (
            <input
              type={f.type || 'text'}
              value={form[f.key] ?? ''}
